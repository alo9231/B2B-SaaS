...
│   ├── page.tsx                # 루트 페이지 (리다이렉트 로직 등)
│   ├── login/                  # 로그인 및 회원가입 페이지
│   │   └── page.tsx            # SignIn/SignUp 폼 조립 및 렌더링
│   └── dashboard/              # 메인 서비스 대시보드 영역
│        ├── layout.tsx         # 대시보드 전용 틀 (Sidebar, Header 포함)
│        ├── page.tsx           # 게시판 메인 페이지 (데이터 리스트)
│        └── PostsDetailModal.tsx # ✅ 대시보드 전용 상세 모달
├── components/                 # 공통 재사용 컴포넌트
│   ├── auth/                   # 인증 관련 컴포넌트
│   │   ├── SignInForm.tsx      # 로그인 폼 (Zod 검증 적용)
│   │   └── SignUpForm.tsx      # 회원가입 폼 (GSAP 애니메이션 적용)
│   ├── layout/                 # 전역 UI 구성 요소
│   │   ├── Header.tsx          # 상단 네비게이션 바
│   │   └── Sidebar.tsx         # 좌측 메뉴 사이드바
│   └── providers/              # 외부 라이브러리 설정 (Wrapper)
│       └── ReactQueryProvider.tsx # React Query 설정
├── lib/                        # 외부 서비스 및 유틸리티 설정
│   └── supabase.ts             # Supabase Client 설정
├── store/                      # 전역 상태 관리 (Zustand)
│   ├── useModalStore.ts        # 모달 제어용 스토어
│   └── useTaskStore.ts         # 게시글/태스크 데이터 스토어
