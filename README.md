...
```text
src/
├── api/                # API 통신 관리 (Axios 인터셉터, 서비스별 엔드포인트)
│   ├── auth.js         # 로그인, 토큰 갱신 등 인증 관련 API
│   └── dashboard.js    # 데이터 통계 및 B2B 대시보드 관련 API
├── assets/             # 정적 리소스 (고해상도 로고, 시스템 폰트, 공통 SCSS)
│   ├── images/         # 서비스 아이콘 및 배경 이미지
│   └── styles/         # 디자인 시스템 가이드 기반 공통 스타일
├── components/         # 아토믹 디자인 기반 재사용 UI 컴포넌트
│   ├── common/         # Table, Pagination, Modal, Chart 등 공통 요소
│   ├── layout/         # Admin Sidebar, Header, Global Layout
│   └── forms/          # 기업 정보 입력, 설정 변경 등 복잡한 폼 컴포넌트
├── hooks/              # 커스텀 훅 (권한 체크, 데이터 페칭 로직 분리)
├── pages/              # 서비스 주요 기능별 페이지
│   ├── Dashboard/      # 데이터 시각화 및 현황 페이지
│   ├── Management/     # 사용자/기업 관리 페이지
│   └── Settings/       # 시스템 설정 및 권한 관리
├── store/              # 전역 상태 관리 (Zustand, Redux, Pinia 등)
├── utils/              # 유틸리티 (데이터 포맷팅, 권한 검사 함수, 엑셀 다운로드 등)
├── App.js              # 라우팅 및 전역 프로바이더 설정
