...
│   │   ├── analytics/        # 데이터 분석/통계 페이지
│   │   ├── board/            # 게시판 기능 (내부 _components 활용)
│   │   └── kanban/           # 칸반 보드 관리 페이지
│   ├── login/                # 로그인 페이지
│   ├── layout.tsx            # Root 레이아웃 (폰트, 전역 설정)
│   ├── page.tsx              # 랜딩 페이지 (Home)
│   └── favicon.ico
├── components/               # 재사용 가능한 컴포넌트
│   ├── auth/                 # 인증 관련 컴포넌트 (LoginForm 등)
│   ├── dashboard/            # 대시보드 전용 UI (PostsDetailModal 등)
│   ├── layout/               # 공통 레이아웃 (Header, Sidebar)
│   └── providers/            # Context Providers (ReactQueryProvider 등)
├── lib/                      # 외부 라이브러리 설정 (supabase.js 등)
├── store/                    # 전역 상태 관리 (Zustand: useModalStore 등)
├── styles/                   # 스타일 관련 파일 (globals.css)
├── types/                    # TypeScript 타입 정의 파일
└── middleware.ts             # 인증 및 라우팅 보호 미들웨어
