...
│   ├── login/                  # - 인증 관련 페이지 (사이드바/헤더 제외 영역)
│   ├── dashboard/              # - 메인 대시보드 비즈니스 로직 및 페이지
│   ├── layout.tsx              # - 최상위 루트 레이아웃 (Server Component / SEO 담당)
│   └── globals.css             # - Tailwind CSS 및 전역 스타일
│
├── components/                 # 🧩 UI Components
│   ├── layout/                 # - 프레임워크 구조 컴포넌트
│   │   ├── Header.tsx          #   : 상단 네비게이션 및 유저 프로필
│   │   ├── Sidebar.tsx         #   : 좌측 메뉴 네비게이션
│   │   └── PageLayoutWrapper.tsx #   : [핵심] 주소별 레이아웃 조건부 렌더링 제어 (Client)
│   ├── ui/                     # - 공통 UI 요소 (Button, Input, Badge 등)
│   └── providers/              # - 글로벌 설정 (React Query, Context API 등)
│
├── utils/                      # 🛠️ Utilities & Services
│   └── supabase/               # - Supabase Client 설정 및 인증 라이브러리
│
├── hooks/                      # ⚓ Custom Hooks (Data Fetching & UI Logic)
├── types/                      # 🏷️ TypeScript Definitions
└── constants/                  # 📌 Static Constants (Menu List, Configs 등)
