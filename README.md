...
```text
src/
├── app/                        # [Routing] 페이지 및 레이아웃
│   ├── layout.tsx              # 최상위 레이아웃 (전역 Provider 설정)
│   ├── page.tsx                # 루트 페이지 (리다이렉트 로직)
│   ├── login/                  # 로그인 및 회원가입
│   │   └── page.tsx            
│   └── dashboard/              # 메인 서비스 영역 (BaaS Dashboard)
│       ├── layout.tsx          # 대시보드 전역 레이아웃 (Sidebar, Header)
│       ├── page.tsx            # 대시보드 메인 (통계 및 요약)
│       └── board/              # ✅ 게시판 도메인 영역 (확장)
...
│           ├── [id]/           # 게시글 상세 및 수정 페이지
│           │   └── page.tsx    
│           └── _components/    # 게시판 전용 컴포넌트 (FileUploader 등)
├── components/                 # [Shared UI] 공통 재사용 컴포넌트
│   ├── auth/                   # 인증 (SignIn/SignUp 폼)
│   ├── layout/                 # 공통 UI (Header, Sidebar)
│   ├── ui/                     # 원자 단위 UI (Button, Input, Badge)
│   └── providers/              # 외부 라이브러리 설정 (ReactQuery, Toast)
├── hooks/                      # [Custom Hooks] 비즈니스 로직 분리
│   ├── usePosts.ts             # 게시글 CRUD 및 페이징 로직
│   └── useFileUpload.ts        # ✅ 파일 업로드 (Supabase Storage)
├── lib/                        # [Config] 서비스 설정 및 유틸리티
│   ├── supabase.ts             # Supabase Client 설정
│   └── utils.ts                # 공통 함수 (날짜 포맷 등)
├── store/                      # [State] 전역 상태 관리 (Zustand)
│   ├── useModalStore.ts        # 모달 제어용
│   └── useBoardStore.ts        # ✅ 게시판 데이터 및 페이징 상태
└── middleware.ts               # 인증 및 경로 보호 미들웨어
